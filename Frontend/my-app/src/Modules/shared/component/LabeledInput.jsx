...
const LabeledInput = ({
  ...
  const handleChange = onChange !== undefined ? onChange : (e) => setText(e.target.value);

  return (
    <div className="w-full font-inter group">
      ...
      >
        {Icon && (
          <Icon
            size={20}
            className="text-SecondOffWhiteText transition-colors group-focus-within:text-MainBlue shrink-0"
            strokeWidth={1.5}
          />
        )}
        ...
        <input
          ...
          className={`bg-transparent border-none outline-none w-full text-white font-jakarta placeholder:text-SecondOffWhiteText ${inputClassName}`}
        ...
        {isPasswordField && (
          ...
          >
            {showPassword ? (
              <Eye size={20} strokeWidth={1.5} className="animate-in fade-in zoom-in duration-200" />
            ) : (
              <EyeClosed size={20} strokeWidth={1.5} className="animate-in fade-in zoom-in duration-200" />
            )}
          </button>
        )}
      </div>
